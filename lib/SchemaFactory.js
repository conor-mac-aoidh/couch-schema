...
SchemaFactory.prototype.getGetMethod = function(ref){
   ...
   * @return object
   **/
  return function(doc, includeSubTypes){
    var tmp = clone(ref);
    // replace subtypes (prefixed with '@' symbol)
    tmp = this._replaceSubTypes(tmp, includeSubTypes);
    ...
    for(j in ref){
      ...
      if(typeof ref[j] === 'object' && ref[j] !== null){
        ...
        // only one sub-document type is allowed
          ...
          if(typeof doc[dj] === 'undefined' || typeof doc[dj].type === 'undefined'){
            errors.push('invalid subdocument, doc of type \'' + match + '\' has missing subdocument \'' + dj + '\'');
          }
      ...
      // if this is not an optional or private key, make sure it exists/type is ok
        ...
        if(!doc.hasOwnProperty(dj)){
          ...
          errors.push('schema invalid, doc' + match + ' has missing property \'' + j + '\'');
        } 
